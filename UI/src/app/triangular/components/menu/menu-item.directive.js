(function() {
    ...
    /* @ngInject */
    function triMenuItemController($scope,$mdDialog, $mdSidenav, $state, $filter, triBreadcrumbsService) {
        ...
        switch(triMenuItem.item.type) {
            case 'dropdown':
                ...
                break;
            case 'link':
                if(triMenuItem.item.dialog)
                    triMenuItem.openLink = openDialog;
                else
        ...
        function openDialog(){
            $mdDialog.show({
                    controller: triMenuItem.item.controller,
                    controllerAs: triMenuItem.item.controllerAs,
                    templateUrl: triMenuItem.item.templateUrl,
                   // targetEvent: $event,
                    locals: {
                        getFocus: false
                    }
        ...
        function openLink() 
        {
            ...
            $state.go(triMenuItem.item.state, params, { reload: true });
